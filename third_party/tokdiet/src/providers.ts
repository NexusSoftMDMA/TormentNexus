...
/**
 * Extract an upstream override from the `x-ctxgov-upstream` header if present.
 */
function upstreamOverride(meta?: RequestMeta): string | undefined {
  const h = meta?.headers['x-ctxgov-upstream'];
  return typeof h === 'string' && h.length > 0 ? stripTrailingSlash(h) : undefined;
}
...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  upstreamBaseUrl(env: NodeJS.ProcessEnv, meta?: RequestMeta): string {
    return (
      upstreamOverride(meta) ||
      stripTrailingSlash(env.TOKDIET_ANTHROPIC_UPSTREAM || env.CTXGOV_ANTHROPIC_UPSTREAM || 'https://api.anthropic.com')
    );
  }
...
export class OpenAIAdapter implements ProviderAdapter {
  ...
  upstreamBaseUrl(env: NodeJS.ProcessEnv, meta?: RequestMeta): string {
    return (
      upstreamOverride(meta) ||
      stripTrailingSlash(env.TOKDIET_OPENAI_UPSTREAM || env.CTXGOV_OPENAI_UPSTREAM || 'https://api.openai.com')
    );
  }
