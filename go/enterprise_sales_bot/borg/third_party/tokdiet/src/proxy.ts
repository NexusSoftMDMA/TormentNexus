...
export function startProxy(opts: StartProxyOptions): ProxyHandle {
  ...
  async function transparentPassthrough(
    ...
    const base = (adapter ?? anthropicAdapter).upstreamBaseUrl(process.env, { method: req.method ?? 'POST', path, headers: inboundHeaders });
  ...
  async function meteredHandle(
    ...
    const upstreamUrl = adapter.upstreamBaseUrl(process.env, meta) + path + query;
  ...
  async function runShadowEval(args: {
    ...
    try {
      ...
      const url = args.adapter.upstreamBaseUrl(process.env, { method: 'POST', path, headers: args.inboundHeaders }) + path + query;
