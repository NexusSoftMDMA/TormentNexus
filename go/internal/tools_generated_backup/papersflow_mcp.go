...
func HandleSearchPapers(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	...
	url := "https://papersflow.com/api/search?q=" + strings.ReplaceAll(query, " ", "+")
	...
	return ok("search results received")
}
...
func HandleGetPaper(ctx context.Context, args map[string]interface{}) (ToolResponse, error) {
	...
	url := "https://papersflow.com/api/paper/" + id
	resp, e := http.DefaultClient.Get(url)
	if e != nil {
		return err("request failed: " + e.Error())
}
	...
	return success("paper details retrieved")
}