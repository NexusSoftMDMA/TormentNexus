...
Borg has two jobs at once:
1. Ship a reliable, truthful, local-first control plane.
2. Preserve a credible long-term vision for richer AI orchestration, multi-agent swarms, and omniscient memory.

This roadmap strictly separates the immediate path to `v1.0.0` stabilization from the expansive `Vision` phase. Borg aims to be a dependable local AI control plane first, and a richer orchestration environment second. That sequencing is the whole game.

---

## ✅ Completed & Stabilized (v1.0.0-alpha.45 Baseline)

*The modular-monolith foundation is officially locked in. P0 is 100% green.*

### 1. Stabilize the CLI Control Surface
- **31 top-level commands**: Full lifecycle management for MCP, Sessions, Providers, Knowledge, Swarm, and Cloud Dev.
- **Truthful Data APIs**: CLI now queries live tRPC/Go sidecar endpoints, completely eliminating all placeholder outputs.
- **MCP Fleet Management**: Multi-process supervision with PID tracking, allowing autonomous background server lifecycles.

### 2. Infrastructure Health & Go Sidecar Parity
- **Borg Doctor**: 10 automated diagnostic checks verifying environment, configuration, and connectivity.
- **Unified Health Snapshot**: `borg info` and `borg status` aggregate telemetry from TS and Go.
- **Go Sidecar Parity**: 543 Go routes active, providing catalog, memory, and routing data to the TS control plane.

### 3. Continuous Integration & Observability
- **73/73 Tests Pass**: Smoke tests, CLI integration, and workflow suites verified against the live runtime.
- **20h+ Runtime Stability**: Verified zero memory leaks or process crashes in long-running 32-core AMD64 production simulations.
- **Dashboard Truthfulness**: 86/86 Next.js pages fully wired to tRPC routers with automatic cache invalidation and native HTTP fallbacks.

---

## 🚀 Next: The Path to v1.0.0 (P1 Targets)

These four high-impact targets represent the immediate operational focus to transition Borg from alpha to a definitive, hardened `v1.0.0` release.

### 1. Progressive Skill Disclosure (Context Hygiene)
*Status: Active Development*
We successfully built the LRU/ranking engine and progressive disclosure for MCP Tools. Now, we are applying that exact same architecture to the Go-native `SkillStore`.
- **The Problem:** Dumping hundreds of operational runbooks and skills into the context window blows out the token budget.
- **The Solution:** Establish a token-budget soft cap for the L1 Scratchpad. Implement BM25/Cosine ranking to silently auto-load skills when the `ToolPredictor` anticipates them, and gracefully evict them via an idle-first LRU policy.

### 2. Migrate `mcp sync` to Go Sidecar (Performance)
*Status: Next in Queue*
- **The Problem:** Detecting and syncing MCP configs for external AI tools (Claude Desktop, Cursor, VS Code) currently sits in the TypeScript control plane, relying on slower Node.js file I/O.
- **The Solution:** Port the detection and syncing logic into `go/internal/mcp/sync.go`. This decouples the TS control plane from disk-heavy polling and solidifies the Go sidecar as the ultimate, hyper-fast authority on the local tool environment.

### 3. Stabilize `agent chat` tRPC Stream (Reliability)
*Status: Next in Queue*
- **The Problem:** Long-running squad missions and deep-research loops are vulnerable to socket flakiness. Stream interruptions currently risk killing context.
- **The Solution:** Implement a robust message buffer on the Go/TS boundary. Queue `A2A_SIGNAL` and `USER_ACTIVITY` events during drops. On the Next.js side, implement an exponential backoff reconnect policy. When the socket re-establishes, the backend gracefully flushes the queue.

### 4. Multi-Model Chatroom & Role Rotation (Swarm/A2A)
*Status: Next in Queue*
- **The Problem:** Foundational A2A negotiation is complete, but true shared-context collaboration between models feels experimental.
- **The Solution:** Tighten the `PairOrchestrator` state machine in Go to strictly enforce the `Planner -> Implementer -> Tester -> Critic` turn cycle. Wire the `getSwarmTranscript` tRPC endpoint directly into the Next.js visualizer so operators can watch models (e.g., Claude, GPT, Gemini) debate and critique implementations in real-time.

---

## 🛠️ Later: P2 / Stabilization

These items must be completed before or shortly after `v1.0.0` but do not block the immediate P1 workflow.

- **Supervisor Tool Prediction:** Fully integrate the `mcp.ToolPredictor` (already in `server.go`) into the active chat prompt chain to preemptively inject tool advertisements.
- **Free-Tier Fallback Expansion:** Explicitly configure routing chains for OpenRouter Free and Google AI Studio native endpoints to ensure ultimate background utility resilience.
- **Browser Extension Deep Injection:** Upgrade the Chrome/Firefox extension beyond manual sync to intercept web chats (Claude.ai, ChatGPT) and expose local MCP tools directly to the browser DOM.
- **Native UI:** Scaffold the replacement for the Electron-based `Maestro` dashboard. The goal is a super lightweight, hyper-fast native binary wrapper (e.g., Wails or Tauri).

---

## 🔮 Vision: The Long-Term Horizon

These concepts ensure Borg remains the bleeding-edge pinnacle of autonomous IDE infrastructure. They are directional R&D goals to pursue once the `v1.0.0` control plane is bulletproof.

1. **Decentralized / P2P Memory Swarm (The "Borg Collective"):** Integrate libp2p or a lightweight WebRTC signaling layer so memory subagents on one machine (e.g., a desktop) can securely broadcast vector embeddings to authorized peers (e.g., a laptop). True omniscient memory across all operator devices.
2. **Kernel-Level File System Watcher (eBPF/FSEvents):**
   Deploy an eBPF daemon to monitor exactly which files are compiling, opening, or crashing, proactively feeding backtraces into the Roundtable Council *before* the developer even asks for debugging help.
3. **Sandboxed Wasm/Deno Execution for MCP Tools:**
   Embed a Rust-based Wasmtime runtime or Deno sandbox inside `@borg/core` to safely execute untrusted public MCP servers with strict network/file capabilities.
4. **Voice-Operated Local LLM (Whisper + Llama Integration):**
   Integrate an offline WebGPU-accelerated Whisper model. Operators can simply speak ("Borg, fix the TS errors in the UI package"), passing commands directly into the execution substrate.

---

## 🚫 Explicit De-Scope (Not in v1.0)

Do not prioritize these ahead of core convergence:
- Economy or payment ideas (e.g., Bobcoin integration).
- Broad, untested P2P federation claims over untrusted networks.
- Major structural rewrites (e.g., full Rust kernel) without measured need.
- Large net-new UI surfaces while core control plane components remain rough.
